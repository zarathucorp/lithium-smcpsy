...
## Date age
df <- lithium$MEDI[, `환자번호` := as.character(`환자번호`)][]
...
data.main <- a %>% 
  ...
  merge(df[, .(avgDose_1day = sum(`함량단위투여량` * `일수`)/sum(`일수`)), by = "환자번호"], by = "환자번호",all.x = T)
...
data.main <- data.main %>% 
  merge(df[`세부검사명`=="Lithium" & as.numeric(`결과`) > 1.0 & (`시행일시` - firstPrescriptionDay >= 0) & (lastPrescriptionDay - `시행일시`  >= 0),  .(LithiumToxicity1.0 = .N), by="환자번호"], by="환자번호", all.x = T) %>% 
  merge(df[`세부검사명`=="Lithium" & as.numeric(`결과`) > 0.8 & (`시행일시` - firstPrescriptionDay >= 0) & (lastPrescriptionDay - `시행일시`  >= 0), .(LithiumToxicity0.8 = .N), by="환자번호"], by="환자번호", all.x = T) %>% 
  merge(df[`세부검사명`=="Lithium" & as.numeric(`결과`) > 1.2 & (`시행일시` - firstPrescriptionDay >= 0) & (lastPrescriptionDay - `시행일시`  >= 0), .(LithiumToxicity1.2 = .N), by="환자번호"], by="환자번호", all.x = T) %>% 
  merge(df[`세부검사명`=="Lithium" & (`시행일시` - firstPrescriptionDay >= 0), .(avgTDM_Lithium = mean(as.numeric(`결과`), na.rm = T)), by="환자번호"], by="환자번호", all.x = T) %>% 
  merge(df[`세부검사명`=="Valproic Acid" & (`시행일시` - firstPrescriptionDay >= 0), .(avgTDM_Valproate = mean(as.numeric(`결과`), na.rm = T)), by="환자번호"], by="환자번호", all.x = T)
...
data.main <- data.main[, .SD, .SDcols = -c("생년월일", "firstPrescriptionDay", "lastPrescriptionDay", "eGFRbelow60Date", "duration", "totDay_Valproate", "totDay_Lithium")]
...
# 처방 정보
df <- lithium$MEDI[, c("환자번호","date","처방명","일수","횟수")]
names(df) <- c("환자번호","date","drug","day","times")
df[, drug := ifelse(drug == "Lithium carbonate 300mg", "Lithium", "Valproate")]
