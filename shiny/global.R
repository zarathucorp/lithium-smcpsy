...
## Exclusion F diagnosis

ICD_data <- readRDS("ICD_data.RDS")
setnames(ICD_data,c("개인정보동의여부","정렬순서"),c("Privacy Consent","NO"))
ICD_data$NO<-ICD_data$NO %>% as.character()

rows.Fdiag <- apply(ICD_data[, .SD, .SDcols = grep("진단코드", names(ICD_data), value = T)], 1, function(x){
  any(substr(x, 1, 3) %in% c(paste0("F0",0:9), paste0("F1",0:9), paste0("F2",0:9), paste0("F3",0:9), paste0("F4",0:8), paste0("F5",0:9),
                             paste0("F6",0:9), paste0("F7",0:9), paste0("F8",0:9), paste0("F9",0:8)))
})

a <- a[NO %in% ICD_data[rows.Fdiag, NO]]
N_profile<-rbind(N_profile,cbind("정신과 F code 있는사람 ",NA,nrow(a)))
...
data.main <- data.main[!((drug == 1 & !is.na(avgTDM_Valproate)) | (drug == 0 & !is.na(avgTDM_Lithium))), -c("NO", "eGFRbelow60Date", "lastTestDate")]  ## NO 제외

#saveRDS(list(data.main = data.main, data.f1 = data.f1, N_profile = N_profile, Year_N = Year_N, eGFRbelow60ratio = eGFRbelow60ratio), "global.RDS")
